...
export function ComicTitle({ children, className, variant = "section" }: ComicTitleProps) {
  if (variant === "hero") {
    return (
      <div className={cn("relative inline-block", className)}>
        ...
        <div className="absolute inset-0 -m-4 bg-(--comic-yellow) -rotate-2 -z-10" />
  ...
  if (variant === "accent") {
    return (
      <div className={cn("relative inline-block", className)}>
        <div className="absolute inset-0 -m-2 bg-(--comic-red) rotate-1 -z-10" />
  ...
  return (
    <div className={cn("relative", className)}>
      ...
      <div className="absolute -bottom-1 left-0 w-1/3 h-2 bg-(--comic-red)" />
    </div>
  )
}
